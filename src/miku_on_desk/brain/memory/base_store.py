...
调用点复用旧的高频访问路径，因此不为假设中的高频访问做额外索引优化。

`find_semantically_similar` 用字符 bigram + ASCII 词 token 的 Jaccard 重叠做相似度启发式，
不引入向量嵌入模型——中文没有天然分词边界，bigram 是不新增依赖前提下最简单可用的近似。

`MemorySystem` 会被 UI 线程（记忆管理面板）和 Brain 线程共享同一个实例，两个线程可能同时对
`index.json` 做读改写。每个公开方法内部用 `threading.RLock()` 互斥——不引入 `filelock`
依赖，跨线程但同进程内的互斥标准库够用；用 RLock 而非 Lock 是因为部分方法（如
`find_semantically_similar`）在持锁的调用路径之外还会调用其他公开方法，需要同线程可重入。
"""
...
class BaseStore:
    ...
    def __init__(
        self, root: Path, *, index_path: Path, default_similarity_threshold: float = 0.80
    ) -> None:
        self._root = root
