...
用 `asyncio.gather` 并发跑三个子提取器，但调度节奏上做了针对性简化：语义/情感两路提取器
"即时触发"，每次 `run_extractions` 调用都会跑；情景提取按"每 6 轮对话或 10 分钟"节奏批量
触发。`.tmp/pending_extractions.jsonl` 队列在本模块里只承担"情景批次累计到第几轮/最早一轮
是什么时候"这一调度状态的跨进程重启持久化——语义/情感两路每次调用都立即处理完毕，不需要跨
调用排队，因此不占用这个队列。
